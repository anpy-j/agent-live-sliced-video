...
function workbuddyModelOptions(provider,selected='auto'){
  return (provider?.models||[]).map(model=>`<option value="${escapeHtml(model.id)}" ${model.id===selected?'selected':''}>${escapeHtml(model.name)}${model.id==='auto'?' · 推荐':''}</option>`).join('');
}
function decisionPanel(job,packet,provider){
  ...
  return `<section class="decision-panel" aria-labelledby="decisionTitle"><div class="decision-head"><div><span class="eyebrow">ACTION REQUIRED</span><h2 id="decisionTitle">需要完成音画编排</h2><p>素材分析已经完成。请选择一种方式提交方案，提交后流程才会继续。</p></div><span class="status waiting_input">等待你的决策</span></div>
    <div class="ai-orchestrator"><div><b>让 LiveCut 调用 WorkBuddy AI</b><p>模型只读取候选摘要并返回结构化编排，不获得文件编辑或终端权限。失败时会停回这里，不会直接渲染错误方案。</p></div><label>编排模型<select id="aiPlanModel">${workbuddyModelOptions(provider,job.model_name||'auto')}</select></label><button class="button primary" id="startAiPlan" ${provider?.available?'':'disabled'}>${provider?.available?'开始 AI 编排':'WorkBuddy CLI 不可用'}</button></div>
    <details class="external-agent"><summary>由外部 Agent 通过 MCP 提交</summary><div class="agent-handoff"><div><p>如果你希望 WorkBuddy、Codex 或 Multica 自己主导，也可以在对应客户端发送下面的指令。</p><code>${escapeHtml(prompt)}</code></div><div class="handoff-actions"><button class="button ghost" id="copyAgentPrompt">复制执行指令</button><a class="button ghost" href="#/mcp">查看 MCP 接入</a></div></div></details>
...
async function renderJob(jobId){
  ...
  const decisionData=canPlan?await Promise.all([api(`/api/jobs/${jobId}/packet`),api('/api/ai/providers')]):[null,null], packet=decisionData[0], provider=decisionData[1]?.providers?.find(x=>x.id==='workbuddy');
  app.innerHTML=`<a href="#/queue" class="back-link">${icons.arrow}返回队列</a><div class="detail-head"><div class="detail-title"><span class="eyebrow">${escapeHtml(job.id)}</span><h1>${escapeHtml(job.title)}</h1><p>${escapeHtml(job.source_path)}</p>${job.model_provider==='workbuddy'?`<small class="model-note">AI 编排：WorkBuddy · ${escapeHtml(job.model_name||'auto')}</small>`:''}</div><div class="detail-actions">${status(job.status)}${canRetry?'<button class="button ghost small" id="retryJob">重新排队</button>':''}${active?'<button class="button danger small" id="cancelJob">取消任务</button>':''}</div></div>
  ...
  ${canPlan?decisionPanel(job,packet,provider):''}
  ...
  if(canPlan){
    ...
    $('#startAiPlan').addEventListener('click',async e=>{const button=e.currentTarget;button.disabled=true;button.textContent='正在加入队列…';try{await api(`/api/jobs/${jobId}/ai-plan`,{method:'POST',body:JSON.stringify({model:$('#aiPlanModel').value})});toast('LiveCut 已开始调用 WorkBuddy AI');renderJob(jobId)}catch(err){toast(err.message);button.disabled=false;button.textContent='开始 AI 编排'}});
...
async function renderSettings(){
  setCrumb('系统设置');loading();const [data,ai]=await Promise.all([api('/api/settings'),api('/api/ai/providers')]),provider=ai.providers.find(x=>x.id==='workbuddy');
  app.innerHTML=`<div class="hero"><div><span class="eyebrow">LOCAL RUNTIME</span><h1>系统设置</h1><p>配置底层切片引擎、AI 提供方、Skill 路径和本地接入策略。</p></div></div><div class="panel"><div class="panel-head"><div><h2>运行配置</h2><p>保存后对新任务生效</p></div></div><form class="settings-form" id="settingsForm"><label>切片引擎目录<input name="engine_path" value="${escapeHtml(data.engine_path||'')}"><small>目录内需要存在 scripts/run_slice.py。</small></label><label>引擎 Python<input name="engine_python" value="${escapeHtml(data.engine_python||'')}"><small>建议使用项目独立的 Python 3.13 环境。</small></label><label>WorkBuddy CLI<input name="workbuddy_cli_path" value="${escapeHtml(data.workbuddy_cli_path||'')}"><small>${provider.available?'已检测到可执行程序。':'当前路径不可用。'}AI 只返回编排 JSON，不允许操作文件。</small></label><label>默认 AI 编排模型<select name="workbuddy_default_model">${workbuddyModelOptions(provider,data.workbuddy_default_model||'auto')}</select><small>新建任务仍可单独修改。</small></label><label>Skill 文件路径<input name="skill_path" value="${escapeHtml(data.skill_path||'')}"></label><label>并行任务数<input name="max_parallel_jobs" type="number" min="1" max="4" value="${data.max_parallel_jobs||1}"><small>第一版实际采用单工作进程，避免视频转码争抢资源。</small></label><label><span><input name="mcp_enabled" type="checkbox" style="width:auto;min-height:0" ${data.mcp_enabled?'checked':''}> 启用 MCP HTTP 入口</span></label><div><button class="button primary" type="submit">保存设置</button></div></form></div>`;
  $('#settingsForm').addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.target);await api('/api/settings',{method:'PUT',body:JSON.stringify({engine_path:f.get('engine_path'),engine_python:f.get('engine_python'),workbuddy_cli_path:f.get('workbuddy_cli_path'),workbuddy_default_model:f.get('workbuddy_default_model'),skill_path:f.get('skill_path'),max_parallel_jobs:Number(f.get('max_parallel_jobs')),mcp_enabled:f.get('mcp_enabled')==='on'})});toast('设置已保存')});
}

async function openNewJob(){
  $('#newJobError').textContent='';
  $('#newJobDialog').showModal();
  try{const ai=await api('/api/ai/providers'),provider=ai.providers.find(x=>x.id==='workbuddy'),select=$('#newJobForm [name="ai_model"]'),selected=(ai.default||'workbuddy:auto').split(':')[1]||'auto';if(provider.available){select.innerHTML=`${workbuddyModelOptions(provider,selected).replaceAll('value="','value="workbuddy:')}<option value="manual">暂不调用 AI · 到编排节点手动决定</option>`}else{select.innerHTML='<option value="manual">WorkBuddy CLI 不可用 · 先手动决定</option>';$('#newJobError').textContent='未检测到 WorkBuddy CLI，可在系统设置中修正路径。';}}catch(err){$('#newJobError').textContent=err.message}
}
function bindCommon(){
