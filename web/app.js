...
async function renderSettings(){
  ...
  $('#settingsForm').addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.target);await api('/api/settings',{method:'PUT',body:JSON.stringify({engine_python:f.get('engine_python'),workbuddy_cli_path:f.get('workbuddy_cli_path'),antigravity_cli_path:f.get('antigravity_cli_path'),codex_cli_path:f.get('codex_cli_path'),opencode_cli_path:f.get('opencode_cli_path'),multica_cli_path:f.get('multica_cli_path'),multica_profile:f.get('multica_profile'),multica_workspace_id:f.get('multica_workspace_id'),ai_default_selection:f.get('ai_default_selection'),visual_ai_default_selection:f.get('visual_ai_default_selection'),skill_path:f.get('skill_path'),mcp_enabled:f.get('mcp_enabled')==='on'})});toast('设置已保存')});
