...
function providerLabel(id){return ({workbuddy:'WorkBuddy',antigravity:'Antigravity',codex:'Codex',manual:'手动'}[id]||id||'手动');}
...
async function renderJob(jobId){
  ...
  const decisionData=canPlan?await Promise.all([api(`/api/jobs/${jobId}/packet`),api('/api/ai/providers')]):[null,null], packet=decisionData[0], ai=decisionData[1];
  app.innerHTML=`<a href="#/queue" class="back-link">${icons.arrow}返回队列</a><div class="detail-head"><div class="detail-title"><span class="eyebrow">${escapeHtml(job.id)}</span><h1>${escapeHtml(job.title)}</h1><p>${escapeHtml(job.source_path)}</p>${job.model_provider&&job.model_provider!=='manual'?`<small class="model-note">AI 编排：${escapeHtml(providerLabel(job.model_provider))} · ${escapeHtml(job.model_name||'auto')}</small>`:''}</div><div class="detail-actions">${status(job.status)}${canRetry?'<button class="button ghost small" id="retryJob">重新排队</button>':''}${active?'<button class="button danger small" id="cancelJob">取消任务</button>':''}</div></div>
  ...
  ${canPlan?decisionPanel(job,packet,ai):''}
...
async function renderSettings(){
  ...
  app.innerHTML=`<div class="hero"><div><span class="eyebrow">LOCAL RUNTIME</span><h1>系统设置</h1><p>配置底层切片引擎、AI 提供方、Skill 路径和本地接入策略。</p></div></div><div class="panel"><div class="panel-head"><div><h2>运行配置</h2><p>保存后对新任务生效</p></div></div><form class="settings-form" id="settingsForm"><label>切片引擎目录<input name="engine_path" value="${escapeHtml(data.engine_path||'')}"><small>目录内需要存在 scripts/run_slice.py。</small></label><label>引擎 Python<input name="engine_python" value="${escapeHtml(data.engine_python||'')}"><small>建议使用项目独立的 Python 3.13 环境。</small></label><div class="settings-section"><b>AI 提供方</b><small>三个 CLI 都以受限模式运行，只接收候选摘要并返回结构化编排。</small></div><label>WorkBuddy CLI<input name="workbuddy_cli_path" value="${escapeHtml(data.workbuddy_cli_path||'')}"><small>${provider('workbuddy').available?'已检测到可执行程序。':'当前路径不可用。'}</small></label><label>Antigravity CLI<input name="antigravity_cli_path" value="${escapeHtml(data.antigravity_cli_path||'')}"><small>${provider('antigravity').available?'已检测到可执行程序。':'当前路径不可用。'}</small></label><label>Codex CLI<input name="codex_cli_path" value="${escapeHtml(data.codex_cli_path||'')}"><small>${provider('codex').available?'已检测到可执行程序。':'当前路径不可用。'}</small></label><label>默认 AI 提供方与模型<select name="ai_default_selection">${providerModelOptions(ai.providers,data.ai_default_selection||ai.default)}</select><small>新建任务仍可单独修改。</small></label><label>Skill 文件路径<input name="skill_path" value="${escapeHtml(data.skill_path||'')}"></label><label>并行任务数<input name="max_parallel_jobs" type="number" min="1" max="4" value="${data.max_parallel_jobs||1}"><small>第一版实际采用单工作进程，避免视频转码争抢资源。</small></label><label><span><input name="mcp_enabled" type="checkbox" style="width:auto;min-height:0" ${data.mcp_enabled?'checked':''}> 启用 MCP HTTP 入口</span></label><div><button class="button primary" type="submit">保存设置</button></div></form></div>`;
  $('#settingsForm').addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.target);await api('/api/settings',{method:'PUT',body:JSON.stringify({engine_path:f.get('engine_path'),engine_python:f.get('engine_python'),workbuddy_cli_path:f.get('workbuddy_cli_path'),antigravity_cli_path:f.get('antigravity_cli_path'),codex_cli_path:f.get('codex_cli_path'),ai_default_selection:f.get('ai_default_selection'),skill_path:f.get('skill_path'),max_parallel_jobs:Number(f.get('max_parallel_jobs')),mcp_enabled:f.get('mcp_enabled')==='on'})});toast('设置已保存')});
}
