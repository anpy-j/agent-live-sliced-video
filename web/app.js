...
function decisionPanel(job,packet,ai){
  ...
  const prompt=`调用 live-slicer 的 get_stage_packet 读取任务 ${job.id}，按照任务包中的约束选择一个最强成片方案，并用 submit_stage_payload 提交 main_product 和 picks。每个 pick 同时标注 product 和 color；默认只做 1 个钩子，不要重复使用同一画面，不要新建任务。`;
...
async function renderSettings(){
  ...
  app.innerHTML=`<div class="hero"><div><span class="eyebrow">SYSTEM CONFIGURATION</span><h1>系统设置</h1><p>配置执行内核、AI 提供方和本地接入策略。</p></div></div><div class="panel model-picker-panel"><div class="panel-head"><div><h2>运行配置</h2><p>内置引擎随应用版本一起升级</p></div></div><form class="settings-form" id="settingsForm"><label>内置切片内核<input value="${escapeHtml(data.engine_path||'')}" readonly><small>当前项目自带，不再依赖外部旧引擎目录。</small></label><label>引擎 Python<input name="engine_python" value="${escapeHtml(data.engine_python||'')}"><small>建议使用项目独立的 Python 3.13 环境。</small></label><div class="settings-section"><b>AI 提供方</b><small>文本编排和多模态混剪可分别指定模型。</small></div><label>WorkBuddy CLI<input name="workbuddy_cli_path" value="${escapeHtml(data.workbuddy_cli_path||'')}"><small>${provider('workbuddy').available?'已检测到可执行程序。':'当前路径不可用。'}</small></label><label>Antigravity CLI<input name="antigravity_cli_path" value="${escapeHtml(data.antigravity_cli_path||'')}"><small>${provider('antigravity').available?'已检测到可执行程序。':'当前路径不可用。'}</small></label><label>Codex CLI<input name="codex_cli_path" value="${escapeHtml(data.codex_cli_path||'')}"><small>${provider('codex').available?'已检测到可执行程序。':'当前路径不可用。'}</small></label><label>OpenCode CLI<input name="opencode_cli_path" value="${escapeHtml(data.opencode_cli_path||'')}"><small>${provider('opencode').available?'已检测到可执行程序。':'当前路径不可用。'}</small></label><label>默认文本编排模型${modelPickerHtml(ai.providers,data.ai_default_selection||ai.default,{name:'ai_default_selection'})}<small>输入关键词可模糊筛选，新建任务仍可单独修改。</small></label><label>默认多模态混剪模型${modelPickerHtml(ai.providers,data.visual_ai_default_selection||ai.visual_default,{name:'visual_ai_default_selection',capability:'vision'})}<small>只显示当前已接入图片输入的模型。</small></label><label>Skill 文件路径<input name="skill_path" value="${escapeHtml(data.skill_path||'')}"></label><label>并行任务数<input name="max_parallel_jobs" type="number" min="1" max="1" value="1" disabled><small>视频转写和渲染固定串行，避免显存与编码资源争抢。</small></label><label class="check-field"><input name="mcp_enabled" type="checkbox" ${data.mcp_enabled?'checked':''}><span>启用 MCP HTTP 入口</span></label><div><button class="button primary" type="submit">保存设置</button></div></form></div><div class="panel"><div class="panel-head"><div><h2>自进化规则</h2><p>版本 ${rules.version||1} · 冲突规则默认禁用</p></div><form id="ruleRollbackForm" class="rollback-form"><input id="ruleRollbackVersion" type="number" min="1" placeholder="版本"><button class="button ghost small">回滚</button></form></div><div class="info-list">${ruleRows}</div></div>`;
  bindModelPickers(app);
  $('#settingsForm').addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.target);await api('/api/settings',{method:'PUT',body:JSON.stringify({engine_python:f.get('engine_python'),workbuddy_cli_path:f.get('workbuddy_cli_path'),antigravity_cli_path:f.get('antigravity_cli_path'),codex_cli_path:f.get('codex_cli_path'),opencode_cli_path:f.get('opencode_cli_path'),ai_default_selection:f.get('ai_default_selection'),visual_ai_default_selection:f.get('visual_ai_default_selection'),skill_path:f.get('skill_path'),mcp_enabled:f.get('mcp_enabled')==='on'})});toast('设置已保存')});
